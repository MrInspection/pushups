...
PushupSchema.methods.updateToday = function () {
  // Get today's date
  const today = moment().startOf("day");

  if (!moment(this.today).isSame(today, "day")) {
    // If so, reset pushups_today and add a new entry to the history array
    this.history.push({
      date: this.today,
      pushups: this.pushups_today,
    });

    this.pushups_today = 0;
    this.today = new Date();
  }
...
PushupSchema.methods.insertPushup = function (pushups) {
  // Get today's date
  const today = moment().startOf("day");
